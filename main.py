...
class Node:
    ...
    def getMax_n_ChildDirection(self):
        nothaveChildren = True
        for i in range(4):
            for j in range(4):
                if self.children[i][j]:
                    nothaveChildren = False
        if nothaveChildren:
            return -1
    ...
    def calculateWeight(self, game_state: typing.Dict) -> float:
        ...
        myBody, enemyBody, myGetFood, enemyGetFood, isCrash = self.simulate(game_state, 0)
        ...
        # 餌への距離を評価に加える（近いほど高評価）
        dist = self.get_dist_to_nearest_food()
        if dist > 0 and dist != 999:
            # 距離が近いほど 0.5 に近い値を加算 (例: 距離1->0.5, 距離5->0.1)
            myWeight += 0.5 / dist
        elif dist == 0:
             # すでに餌の上にいる
            myWeight += 0.6
        ...
        #incDif = max(0, min((incMyLength - incEnemyLength) * 0.1, 0.3))
        ...
        if not (2 <= self.myBody[0][0] < fieldHeight- 2 and 2 <= self.myBody[0][1] < fieldWidth - 2):
            myWeight -= 0.2
        if not (2 <= self.enemyBody[0][0] < fieldHeight- 2 and 2 <= self.enemyBody[0][1] < fieldWidth - 2):
            enemyWeight -= 0.2

        #生存範囲で評価
        """
        cashPoint, myPoint, enemyPoint = deepCalculateSurvivalArea(myBody, enemyBody)
    ...
    def get_dist_to_nearest_food(self):
        head = self.myBody[0]
        foods = self.foodPos
        ...
        obstacles = set(self.myBody) | set(self.enemyBody)
        if head in obstacles: obstacles.remove(head)
        ...
        while queue:
            (curr_y, curr_x), dist = queue.popleft()
            if (curr_y, curr_x) in foods:
                return dist

            for dy, dx in directions:
                ny, nx = curr_y + dy, curr_x + dx

                if 0 <= ny < fieldHeight and 0 <= nx < fieldWidth:
...
def directionCheck_myMove(game_state):
    myBody = get_myBody(game_state)
    enemyBody = get_enemyBody(game_state)
    
    for i in direction.values():
        myDirectionOk, tempene, tempcrash = directionCheck_notBoard(myBody, enemyBody, i, -1)
        if myDirectionOk:
            return i
    return 0
...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    if next_direction == -1:
        next_direction = directionCheck_myMove(game_state)
